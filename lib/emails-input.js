...
const EmailsInput = function(node, props) {
    ...
    node.appendChild(elementFactory('div', { class: 'emails-container' }, { click: clickContainer }, [
        ...
        elementFactory('input', { type: 'text', class: 'input-email', placeholder: 'add more people...' }, { keypress: keyPressInput, focusout: focusOut, paste: emailsPaste })
    ]));
    ...
    //Additional prop to get an array of valid emails and exacute a callback function
    if (props.hasOwnProperty('shareButton') && typeof props.shareButton === 'function') {
        const shareButton = function() {
            const emailsArray = [];
            const validEmailsNodes = node.querySelectorAll('.emails-container__email-block');
            for (var i = 0; i < validEmailsNodes.length; i++) {
                emailsArray.push(validEmailsNodes[i].querySelector('.emails-container__email-block__email-text-container').childNodes[0].nodeValue);
            }
            props.shareButton(emailsArray);
        }
        node.appendChild(
            elementFactory('input', { type: 'button', class: 'emails-container__button', value: 'Share' }, { click: shareButton })
        );
    }
